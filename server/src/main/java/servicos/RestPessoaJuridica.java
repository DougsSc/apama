...
public class RestPessoaJuridica extends Network implements HttpHandler {

    private static final Logger logger = LogManager.getLogger(RestPessoaJuridica.class.getName());
    private Erro erro;
        ...
        try {
            switch (he.getRequestMethod().toUpperCase()) {
                case METHOD_POST:
                    // CONVERT json
                    try {
                        pessoa = gson.fromJson(body, PessoaJuridica.class);
                        ...
                        pessoa.setCnpj(Utilidades.somenteNumeros(pessoa.getCnpj()));
                        ...
                        // RESPONSE CREATED
                        logger.info("Pessoa Jurídica " + pessoa.getId() + " recebida!");
                ...
                case METHOD_DELETE:
                    ...
                    pessoa = new PessoaJuridicaDAO().busca(Integer.parseInt(parameters.get("id").get(0)));
                    if(pessoa == null){
...
//                headers.set(HEADER_ALLOW, Arrays.toString(ALLOWED_METHODS));
    ...
    private boolean validaDados(PessoaJuridica pessoa){
        if(pessoa == null){
            ...
            return false;
        }else if(!Validacao.validarCNPJ(Utilidades.somenteNumeros(pessoa.getCnpj()))){
            this.erro = new Erro("PJ.01");
            return false;
        }
    ...
    private boolean validaPessoa(PessoaJuridica pessoa){
        if(pessoa.getRazaoSocial().trim().isEmpty()){
            return false;
        }else if(pessoa.getNomeFantasia().trim().isEmpty()){
            return false;
        }else if(Utilidades.somenteNumeros(pessoa.getCnpj()).isEmpty()){
            return false;
        }
